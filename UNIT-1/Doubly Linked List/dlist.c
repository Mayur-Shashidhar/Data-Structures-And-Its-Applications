		...
		void delete_first(dlist_t *ptr_list)
		{
			...
			{		
			         ...
			         if(pres->next==NULL) // only one node 
			           ...
			           {
                         pres->next->prev=NULL; // make prev of next node as NULL
			...
			else   
			{
			 ...
			 else
			 {
				 ...
				 pres->prev->next=NULL; //make next pointer of previous node as NULL
			 }
			 free(pres);
		}
	 }
 ...
 void insert_after(dlist_t* ptr_list, int value , int key)
	{
		...
		if(pres->key==value)
		{
			if(pres->next!=NULL) // anywhere before the last node
			{
				temp->next=pres->next;
				pres->next->prev=temp;
				pres->next=temp;
			...
			{
				pres->next=temp; // after the last node
				temp->prev=pres;
			}
		}
			else
			{
				printf("\nNode with a value not found..\n");
				free(temp);
			}
	}
	...
	void insert_before(dlist_t* ptr_list, int value , int key)
	{
		...
		if(pres->key==value) // key found
		{
			if(pres->prev==NULL) // first node
			{
				temp->next=pres;
				pres->prev=temp;
				ptr_list->head=temp;
